...
from auth_service.app.core.dependencies import get_current_user, require_permission
...
async def create_role(
    ...
    try:
        ...
        logger.error("Ошибка при создании роли", detail=str(e), role_name=role_data.name)
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(e)) # Conflict if role exists
...
async def assign_role_to_user(
    ...
    if not assigned:
        logger.warning("Не удалось назначить роль пользователю", user_id=user_id, role_id=role_id)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User or Role not found, or role already assigned")
...
async def revoke_role_from_user(
    ...
    if not revoked:
        logger.warning("Не удалось отозвать роль у пользователя", user_id=user_id, role_id=role_id)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User-Role assignment not found")
